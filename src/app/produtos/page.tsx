...
// Utilitário Sênior para sanitizar entradas monetárias (evita NaN no banco)
...
export default function ProdutosPage() {
  ...
  const [products, setProducts] = useState<any[]>([]); // any temporário até atualizar o schema global
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!tenantId) return; // Trava de segurança para o TypeScript
    ...
    try {
      if (editingId) {
        // (supabase as any) cala o alerta da coluna nova
  ...
  const handleDelete = async (id: string, productName: string) => {
    if (!tenantId) return; // Trava de segurança
  ...
  const toggleWebsiteVisibility = async (id: string, currentStatus: boolean) => {
    ...
    try {
      ...
      fetchProducts(); // Reverte a UI em caso de falha no banco
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-sm font-bold text-slate-200 outline-none focus:border-green-500 appearance-none cursor-pointer"
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text" required value={costPrice} onChange={(e) => setCostPrice(e.target.value)}
                  placeholder="0,00"
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-sm font-mono font-black text-red-500 outline-none focus:border-red-500 placeholder:text-slate-700"
                />
              </div>
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-2 block">Venda (R$)</label>
                <input
                  type="text" required value={basePrice} onChange={(e) => setBasePrice(e.target.value)}
                   placeholder="0,00"
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-sm font-mono font-black text-green-500 outline-none focus:border-green-500 placeholder:text-slate-700"
                />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 pt-2">
              ...
              <label className={`flex items-center gap-3 p-4 border rounded-2xl cursor-pointer transition-all ${showOnWebsite ? 'bg-[#F6C689]/10 border-[#F6C689]/30' : 'border-slate-800 hover:bg-slate-800/30'}`}>
                <input
                  type="checkbox" checked={showOnWebsite} onChange={(e) => setShowOnWebsite(e.target.checked)}
                  className="w-5 h-5 rounded border-slate-700 bg-slate-950 accent-[#F6C689]"
                />
        ...
        <div className="lg:col-span-2 space-y-6 order-1 lg:order-2">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-2 flex items-center shadow-xl relative">
            ...
            />
            <div className="pr-4 text-[10px] font-black uppercase tracking-widest text-slate-500 bg-slate-950 py-2 px-3 rounded-xl border border-slate-800">
                    ...
                    return (
                      <tr key={product.id} className="hover:bg-slate-800/40 transition-colors group">
                        <td className="p-5">
                          <div className="flex items-center gap-3">
                            ...
                            {product.show_on_website && (
                              <span className="bg-[#F6C689]/10 text-[#F6C689] border border-[#F6C689]/20 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest" title="Visível no site">
                                No Site
                              </span>
                            )}
                          </div>
                          <div className="flex gap-2 mt-2">
                            <span className="bg-slate-950 border border-slate-800 px-2 py-0.5 rounded text-[9px] text-slate-500 font-bold uppercase tracking-widest">
                              {calcLabels[product.calculation_type]}
                            </span>
                            {product.is_outsourced && (
                              <span className="bg-orange-500/10 border border-orange-500/20 px-2 py-0.5 rounded text-[9px] text-orange-500 font-bold uppercase tracking-widest">
                                Terceirizado
                              </span>
                            )}
                          </div>
                        ...
                        <td className="p-5 text-right">
                          <span className={`font-mono font-black text-xs px-2 py-1 rounded bg-slate-950 border border-slate-800 ${margemColor}`}>{margem.toFixed(1)}%</span>
                        </td>
                        <td className="p-5">
                          <div className="flex items-center justify-center gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                            {/* Ação Rápida: Vitrine */}
                            <button 
                              onClick={() => toggleWebsiteVisibility(product.id, product.show_on_website)} 
                              className={`p-2 border rounded-xl transition-all ${product.show_on_website ? 'bg-[#F6C689]/10 text-[#F6C689] border-[#F6C689]/30 hover:bg-[#F6C689]/20' : 'bg-slate-950 border-slate-800 text-slate-500 hover:text-[#F6C689] hover:border-[#F6C689]/50'}`} 
                              title={product.show_on_website ? "Remover do site" : "Colocar no site"}
                            >
